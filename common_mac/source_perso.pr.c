/* Process model C form file: source_perso.pr.c */
/* Portions of this file copyright 1992-2007 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char source_perso_pr_c [] = "MIL_3_Tfile_Hdr_ 120A 30A opnet 7 4A5DDDCF 4A5DDDCF 1 lefkada-laptop theoleyr 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 11ab 5                                                                                                                                                                                                                                                                                                                                                                                                    ";
#include <string.h>
...
#define FIN_PREAMBLE_DEC	source_perso_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((source_perso_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
extern "C" {
#endif
	void source_perso (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_source_perso_init (int * init_block_ptr);
	void _op_source_perso_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_source_perso_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_source_perso_alloc (VosT_Obtype, int);
	void _op_source_perso_svar (void *, const char *, void **);
...
void
source_perso (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	...
	FIN_MT (source_perso ());
		...
		FSM_ENTER_NO_VARS ("source_perso")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_UNFORCED_NOLABEL (0, "init", "source_perso [init enter execs]")
				FSM_PROFILE_SECTION_IN ("source_perso [init enter execs]", state0_enter_exec)
				{
			...
			FSM_EXIT (1,"source_perso")


			/** state (init) exit executives **/
			FSM_STATE_EXIT_UNFORCED (0, "init", "source_perso [init exit execs]")


			/** state (init) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "on", "source_perso [init -> on : default / ]")
				/*---------------------------------------------------------*/



			/** state (on) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "on", state1_enter_exec, "source_perso [on enter execs]")
				FSM_PROFILE_SECTION_IN ("source_perso [on enter execs]", state1_enter_exec)
				{
				//----------- Traffic generation  -----------------
				double				arg1_pk_time;
				double				time_start;
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (3,"source_perso")


			/** state (on) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "on", "source_perso [on exit execs]")
				FSM_PROFILE_SECTION_IN ("source_perso [on exit execs]", state1_exit_exec)
				{
			...
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "on", "on", "source_perso [on -> on : default / ]")
				/*---------------------------------------------------------*/
...
void
_op_source_perso_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_source_perso_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_source_perso_terminate ())
...
/* local variable prs_ptr in _op_source_perso_svar function. */
#undef pid_string
#undef debug_mode
...
VosT_Obtype
_op_source_perso_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_source_perso_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (source_perso)",
		sizeof (source_perso_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_source_perso_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	source_perso_state * ptr;
	FIN_MT (_op_source_perso_alloc (obtype))

	ptr = (source_perso_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "source_perso [init enter execs]";
#endif
		}
...
void
_op_source_perso_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	source_perso_state		*prs_ptr;

	FIN_MT (_op_source_perso_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (source_perso_state *)gen_ptr;
