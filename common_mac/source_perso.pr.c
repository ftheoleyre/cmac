...
static const char source_perso_pr_c [] = "MIL_3_Tfile_Hdr_ 81A 30A modeler 7 4483EB46 4483EB46 1 ares-theo-1 ftheoley 0 0 none none 0 0 none 0 0 0 0 0 0                                                                                                                                                                                                                                                                                                                                                                                                                 ";
#include <string.h>
...
#define pr_state_ptr            		((source_perso_state*) SimI_Mod_State_Ptr)
...
#undef FIN_PREAMBLE
#define FIN_PREAMBLE	source_perso_state *op_sv_ptr = pr_state_ptr;
...
extern "C" {
#endif
	void source_perso (void);
	Compcode source_perso_init (void **);
	void source_perso_diag (void);
	void source_perso_terminate (void);
	void source_perso_svar (void *, const char *, char **);
#if defined (__cplusplus)
} /* end of 'extern "C"' */
#endif
...
source_perso (void)
	...
	if (1)
		...
		FSM_BLOCK_SWITCH
			...
			FSM_STATE_ENTER_UNFORCED (1, state1_enter_exec, "on", "source_perso () [on enter execs]")
				{
				//----------- Traffic generation  -----------------
				double				arg1_pk_time;
				double				time_start;
...
Compcode
source_perso_init (void ** gen_state_pptr)
	{
	int _block_origin = 0;
	static VosT_Address	obtype = OPC_NIL;

	FIN (source_perso_init (gen_state_pptr))

	if (obtype == OPC_NIL)
		{
		/* Initialize memory management */
		if (Vos_Catmem_Register ("proc state vars (source_perso)",
			sizeof (source_perso_state), Vos_Vnop, &obtype) == VOSC_FAILURE)
			{
	...
	*gen_state_pptr = Vos_Catmem_Alloc (obtype, 1);
	if (*gen_state_pptr == OPC_NIL)
		...
		((source_perso_state *)(*gen_state_pptr))->current_block = 0;
...
void
source_perso_terminate (void)
	{
	int _block_origin = __LINE__;

	FIN (source_perso_terminate (void))
...
/* local variable prs_ptr in source_perso_svar function. */
#undef pid_string
#undef debug_mode
...
void
source_perso_svar (void * gen_ptr, const char * var_name, char ** var_p_ptr)
	{
	source_perso_state		*prs_ptr;

	FIN (source_perso_svar (gen_ptr, var_name, var_p_ptr))
