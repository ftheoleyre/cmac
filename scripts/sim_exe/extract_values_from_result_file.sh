...
#parameters
echo "cat $MAC_FILE |  grep \"Number of nodes\"  |cut -d \":\" -f 2"
...
then
	...
	FILE_RESULT_AGGREG="$3/nodes=`echo $NB_NODES`_length=`echo $SIM_LENGTH`_ktree=`echo $KTREE_ALGO`_nbchannels=`echo $NB_CHANNELS`.txt"
